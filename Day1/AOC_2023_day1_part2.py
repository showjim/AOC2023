...
def sum_calibration_values(text):
    ...
    # Compile a regex pattern to match whole words only
    number_word_pattern = re.compile('|'.join(number_words.keys()))
    number_word_pattern_rev = re.compile('|'.join(number_words_rev.keys()))

    # Split the text into lines
    text_lines = text.split('\n')

    for line in text_lines:
        original_line = line  # Save the original line for debugging
        line_rev = "".join(reversed(line)) 

        # Replace whole number words with digits
        def replace_with_digit(match):
            word = match.group(0)
            return number_words[word]
        
        def replace_with_digit_rev(match):
            word = match.group(0)
            return number_words_rev[word]

        line = number_word_pattern.sub(replace_with_digit, line)
        line_rev = number_word_pattern_rev.sub(replace_with_digit_rev, line_rev)

        # Find all digits in the current line
        digits = re.findall(r'\d', line)
        digits_rev = re.findall(r'\d', line_rev)
        if digits and digits_rev:
            # Combine the first and last digit to form a two-digit number
            calibration_value = int(digits[0] + digits_rev[0])
            total_sum += calibration_value
            # Debug output
            print(f"Line: '{original_line}' -> Converted: '{line}' -> Digits: {digits} -> Reversed Digits: {digits_rev} -> Calibration value: {calibration_value}")
...
sum_of_values = sum_calibration_values(text)
print(sum_of_values)