    ...
    for match in pattern.finditer(game_data):
        ...
        for subset in cubes:
            ...
            for color in ['red', 'green', 'blue']:
                ...
                if color_match:
                    counts[color] = max(counts[color], int(color_match.group(1)))
...
def cal_part2_result(games):
    ...
    for game_id, subsets in games.items():
        ...
        for subset in subsets:
            for color in ['red', 'green', 'blue']:
                max_counts[color] = max(max_counts[color], subset[color])
        print(f"Game {game_id} is: {max_counts['red'] * max_counts['green'] * max_counts['blue']}")
...
part2_result = cal_part2_result(games)
print(f"The part 2 result is: {part2_result}")